<?php
    ...
    if (isset($_POST['intransit'])) {
        ...
        $epayment_process = "UPDATE upti_transaction SET trans_status = 'In Transit' WHERE id = '$id'";
        $epayment_process_qry = mysqli_query($connect, $epayment_process);

        $epayment_process1 = "UPDATE upti_order_list SET ol_status = 'In Transit' WHERE ol_poid = '$poid'";
    ...
    ?>
        <script>alert('Order Status has been changed to In Transit Successfully');window.location.href = '../poid-list.php?id=<?php echo $id ?>';</script>
    <?php
        }
    ?>