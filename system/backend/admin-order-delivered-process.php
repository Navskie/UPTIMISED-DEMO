<?php
    ...
    if (isset($_POST['delivered'])) {
        ...
        //         // added stockist percentage
                echo
                echo $percent = $ol_php * 0.03;

                $stockist_wallet = "SELECT * FROM stockist_wallet WHERE w_id = '$stockist'";
                $stockist_wallet_qry = mysqli_query($connect, $stockist_wallet);
                $stockist_balance = mysqli_fetch_array($stockist_wallet_qry);
                echo '<br>';
                echo $s_wallet = $stockist_balance['w_earning'] + $percent;
                echo '<br>';
                echo '<br>';
                // $update_stockist_wallet = mysqli_query($connect, "UPDATE stockist_wallet SET w_earning = '$s_wallet' WHERE w_id = '$stockist'");
        ...
        if ($country == 'CANADA' && $state == 'ALBERTA' || $country == 'USA' || $country == 'TAIWAN') {
          ...
          while ($order = mysqli_fetch_array($order_list)) {
            echo $item_code = $order['ol_code'];
            echo '<br>';
            echo $item_qty = $order['ol_qty'];
            echo '<br>';
            echo $item_php = $order['ol_php'];
            echo '<br>';
            echo $subtotal = $order['ol_subtotal'];
            echo '<br>';
            ...
            if (mysqli_num_rows($pack_stmt) > 0) {
              ...
              $_q5 = $pack['package_five_qty'];
                ...
                echo $n_q1 = $_q1 * $item_qty;
                echo '<br>';
                echo $n_q2 = $_q2 * $item_qty;
                echo '<br>';
                echo $n_q3 = $_q3 * $item_qty;
                echo '<br>';
                echo $n_q4 = $_q4 * $item_qty;
                echo '<br>';
                echo $n_q5 = $_q5 * $item_qty;
                echo '<br>';

              echo $buy = ($stockist_price * $n_q1) + ($stockist_price2 * $n_q2) + ($stockist_price3 * $n_q3) + ($stockist_price4 * $n_q4) + ($stockist_price5 * $n_q5);
              echo '<br>';
              echo $item_php;
              echo '<br>';
              echo $refund = $buy - $item_php;
              // echo '<br>';
