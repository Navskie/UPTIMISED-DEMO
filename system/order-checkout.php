<?php
    ...
    // LESS shipping FEE
    $less_shipping_sql = "SELECT SUM(ol_qty) AS abc FROM upti_order_list INNER JOIN upti_code ON code_name = ol_code 
    WHERE 
    ...
    $one_to_3 = "SELECT SUM(ol_qty) AS abc, COUNT(ol_qty) AS de FROM upti_order_list INNER JOIN upti_code ON code_name = ol_code 
    WHERE 
    ol_poid = '$poid' AND code_category = 'ONE TO THREE'";
    $one_to_3_qry = mysqli_query($connect, $one_to_3);
    $one_qury = mysqli_num_rows($one_to_3_qry);
    $one_to_3_fetch = mysqli_fetch_array($one_to_3_qry);

    $bilangka3 = $one_to_3_fetch['de'];

    if ($bilangka3 > 0) {
      $getskona_trial_3 = $one_to_3_fetch['abc'];
      $passed = 'pasado';
    } else {
      $getskona_trial_3 = 0;
      $passed = 'bagsak';
    ...
    $getskona = $less_shipping_fetch['abc'] * $getskona_trial_3;
    // echo '<br>';
    $getskona_3 = $less_shipping_fetch['abc'] * 3;
    // echo '<br>';
    // echo $passed;

    // if ($getskona == $getskona_3 && $passed == 'pasado' || $passed = 'bagsak' && $getskona_trial_3 == 0) {
    //   echo 'passs';
    // } else {
    //   echo 'fail';
    // }
    ...
    if ($getskona == $getskona_3 && $passed == 'pasado' || $passed = 'bagsak' && $getskona_trial_3 == 0) {
          ...
          if ($check_boga_num != 1 && $free != 1 || $check_boga_num != 0 && $free != 1) {
              ...
              if ($get_num_rebatable > 0 && $check_n_r > 0 || $get_num_rebatable > 0 && $check_n_r == 0) {
                  ...
                  // CHECK REBATABLE
                      ...
                      // Total Amount
                      $total_amount = $subtotal + $shipping + $surcharge - $less_shipping_fee;
      ...
      flash("warning", "Ooops! You’ve exceeded the maximum purchase quantity!");
      header('location: order-list.php');
    }
